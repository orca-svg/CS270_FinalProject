#!/usr/bin/env python3
"""Hand gesture controller for LEGO SPIKE Prime over Pybricks BLE (Windows Threaded Version).
...
def ensure_hand_landmarker_model(model_path: Path, model_url: str) -> Path:
    ...
    try:
        ...
        raise RuntimeError("Could not download the MediaPipe Hand Landmarker model.") from exc
    return model_path
...
def hand_to_speed(hand_x: int, hand_y: int, frame_w: int, frame_h: int, args: argparse.Namespace, fist: bool) -> HandState:
    ...
    pan = 0 if abs(dx) < args.deadzone_px else int(clamp((dx / (frame_w / 2)) * args.max_pan_speed * args.gain, -args.max_pan_speed, args.max_pan_speed))
    tilt = 0 if abs(dy) < args.deadzone_px else int(clamp((-dy / (frame_h / 2)) * args.max_tilt_speed * args.gain, -args.max_tilt_speed, args.max_tilt_speed))
...
def draw_overlay(frame, state: Optional[HandState], last_command: str, mirror: bool) -> None:
    ...
    if state is None:
        ...
        cv2.putText(frame, f"dx={state.dx} dy={state.dy} M,{state.pan},{state.tilt}", (12, 62), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (230, 230, 230), 2)

    cv2.putText(frame, f"q quit | mirror={'on' if mirror else 'off'} | WIN THREAD", (12, h - 42), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
...
# =====================================================================
def ble_worker(args: argparse.Namespace, cmd_queue: queue.Queue, stop_event: threading.Event):
            ...
            try:
                ...
                while not cmd_queue.empty():
                    ...
                    if next_cmd == "STOP" or cmd == "STOP":
                        ...
                        # 최신 명령어로 덮어씌웁니다 (fire latch가 유실되지 않도록 체크)
                        # cmd가 M,pan,tilt,1 처럼 발사(fire) 명령인 경우, next_cmd가 0이더라도 발사 명령은 유지하거나 병합할 수 있습니다.
                        if "M," in cmd and "M," in next_cmd:
                            parts_cmd = cmd.split(",")
                            parts_next = next_cmd.split(",")
                            # 만약 이전 명령이나 새 명령 중 하나라도 발사(fire=1) 신호가 있으면 유지
...
def run_camera(args: argparse.Namespace) -> None:
    ...
    ble_thread = threading.Thread(target=ble_worker, args=(args, cmd_queue, stop_event), daemon=True)
    ble_thread.start()

    camera = OpenCVCamera(args.camera, args.width, args.height)
    detector = create_hand_landmarker(args)

    cv2.namedWindow("Gesture BT Controller (Win)", cv2.WINDOW_NORMAL)
    last_send_time = 0.0
    last_command = "ready"
    ...
    print("=====================================================")
    print(" 🚀 Windows Multi-Thread Controller Started")
    print("=====================================================")
    ...
    try:
        while not stop_event.is_set():
            ...
            cv2.imshow("Gesture BT Controller (Win)", frame)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--mirror", action="store_true", default=True, help="Mirror the camera image.")
    p.add_argument("--no-mirror", action="store_false", dest="mirror", help="Do not mirror the camera image.")
    ...
    # Changed default model path resolution since the file is in the same dir as the original script
