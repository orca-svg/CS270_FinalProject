...
See the [local voice guide](docs/LOCAL_VOICE.md) for MLX Whisper on Apple
Silicon, faster-whisper on Windows, and parallel camera/voice execution.
See [Voice-BLE-Hub Safety](docs/VOICE_BLE_SAFETY.md) for the heartbeat,
fail-safe, reconnect, and Hub validation design.

## Voice-BLE-Hub Safety

- Camera controllers always start in `safe`.
- Voice writers hold a single-writer OS lock, write `safe` on startup/shutdown,
  and update a heartbeat every 2 seconds.
- Voice state expires to `safe` after 10 seconds and requires a new command to recover.
- `single` and `burst` require confidence >= 0.60.
- BLE reconnects discard `fire=1` and restart the 0.4-second target confirmation.
- Windows mode changes purge queued fire requests.
- Hub connections validate the expected server, A/B/C/D/F ports, `RUNNING`, and `rdy`.
- A/B launcher wheels continue spinning in `safe`; stop the Hub program before physical work.

Run camera and voice in separate terminals using the same absolute
`--control-mode-file` path.
...
- `gesture_bt/voice_commander.py`: Voice-to-JSON bridge with `옵티머스` wake-word gating and `--dry-run-text` verification.
- `gesture_bt/voice_commander_local.py`: Local MLX/faster-whisper entry point.
- `gesture_bt/local_speech.py`: Shared microphone VAD and platform backends.
...
# Terminal 2: voice commander waits for '옵티머스', then writes command -> control_mode.json
