...
로컬 음성 인식의 macOS MLX/Windows faster-whisper 설치와 카메라 병렬 실행 절차는
[로컬 음성 인식 실행 가이드](docs/ko/LOCAL_VOICE.md)를 따른다.
이번에 결정한 heartbeat, 자동 SAFE, BLE 재연결, Hub 검증의 적용 내역과 사용자가
수행한 작업은 [음성-BLE-Hub 안전성 보강 기록](docs/ko/VOICE_BLE_SAFETY.md)에 정리했다.

## 🔒 음성-BLE-Hub 안전 규칙

```text
음성 -> control_mode.json -> 카메라 발사 정책 -> BLE -> Prime Hub
```

- 카메라 컨트롤러는 항상 `SAFE`로 시작한다.
- 음성 프로세스는 모델 로딩 전과 종료 시 `SAFE`를 기록하고 2초마다 heartbeat를 쓴다.
- heartbeat가 10초 이상 끊기면 자동 `SAFE`가 되며, 새 음성 명령 전에는 복구되지 않는다.
- 호출어는 `옵티머스`이며 `Optimus` 영문 전사도 허용한다.
- `single`/`burst`는 confidence 0.60 이상에서만 적용한다.
- BLE 단절·재연결 시 기존 `fire=1`을 폐기하고 표적을 0.4초부터 다시 확인한다.
- Windows는 모드 변경 때 BLE 큐의 발사 요청을 제거한다.
- Hub 서버 버전, A/B/C/D/F, `RUNNING`, `rdy` 검증 실패 시 `y/N` 확인을 요구한다.
- `SAFE`에서도 A/B 발사 휠은 계속 회전한다. 물리 작업 전 Hub 프로그램을 정지한다.

두 프로세스는 같은 절대 JSON 경로를 사용해야 한다.

```bash
# 터미널 1: macOS 카메라 + Hub
python balloon_intercept.py --hub-name "Team5" \
  --control-mode-file "$(pwd)/control_mode.json" --fire-debug

# 터미널 2: macOS MLX 음성
python voice_commander_local.py --backend mlx --language ko \
  --control-mode-file "$(pwd)/control_mode.json"
```

Windows에서는 각각 `balloon_intercept_win.py`와 다음 음성 명령을 사용한다.
...
- `[MODE] ... -> safe`: 음성 중지, TTL 만료 또는 안전 명령 적용
- `[MODE] rejected ... confidence`: 발사 confidence 0.60 미달
- `[RUN] BLE/Hub recovered`: 재연결 후 발사 폐기와 표적 잠금 초기화
- `[HUB-VALID]`: 서버 버전과 모터 포트 검증 결과
- `[STALE]`: BLE는 연결됐지만 Hub 응답이 없는 상태
...
- `gesture_bt/voice_commander_local.py`: MLX/faster-whisper 로컬 음성 인식 진입점.
- `gesture_bt/local_speech.py`: 공통 마이크 VAD와 플랫폼별 Whisper 백엔드.
...
기본값은 오작동 방지를 위해 먼저 `옵티머스` 호출어를 들은 뒤 다음 문장을 명령으로 처리한다.
...
# 기본 동작: "옵티머스" 호출어 후 다음 문장을 명령으로 처리
