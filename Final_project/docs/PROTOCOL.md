...
Legacy files may contain only `mode`, but active voice writers include command
identity and heartbeat fields:

```json
{
  "mode": "single",
  "session_id": "voice-process-uuid",
  "command_id": "voice-command-uuid",
...
Voice writers update `heartbeat_at` every 2 seconds without changing
`command_id` or `updated_at`. Controllers start in `safe`, expire state after
10 seconds, and require a new `command_id` after expiry. Writes use a temporary
file and atomic `os.replace()`.
...
sender waits for readiness before sending the next packet, validates the server
version and A/B/C/D/F ports, can remote-start a stopped saved program, and
reconnects/rescans after BLE link loss. Recovery replay is aim-only; `fire=1`
is discarded and target confirmation restarts from 0.4 seconds.
